...
unsigned int createRainDrops(unsigned int count);
...
//rain
const unsigned int raindropCount = 10000;
const float DRAW_DISTANCE = 10.0f; //box size
const float LINE_LENGTH = 0.001f;
const glm::vec3  VELOCITY(0, -.175f, 0);
const glm::vec3  GRAVITY(0, -8.0f, 0);
const glm::vec3  WIND(.5f, 0, .5f);
glm::mat4 oldViewProj = glm::mat4(1.0f);
glm::vec3 prevCameraPosition = camPosition;
...
void drawObjects(){
    ...
    if (oldViewProj == glm::mat4(1.0f)){
        //std::cout << "initial \n";
        oldViewProj = viewProjection;
    }
    ...
    oldViewProj = viewProjection;
    prevCameraPosition = camPosition;
}
...
void drawRain(glm::mat4 model){
    ...
    rainShaderProgram->setFloat("currentTime", currentTime);
    rainShaderProgram->setFloat("drawDistance", DRAW_DISTANCE);
    rainShaderProgram->setFloat("lineLength", LINE_LENGTH);
    rainShaderProgram->setVec3("velocity", VELOCITY);
    rainShaderProgram->setVec3("cameraPosition", camPosition);
    rainShaderProgram->setVec3("prevCameraPosition", prevCameraPosition);
    rainShaderProgram->setMat4("model", model);
    rainShaderProgram->setMat4("prevModel", oldViewProj);

    auto offsets = glm::vec3 ( GRAVITY + WIND) * currentTime ;
    offsets -=  camPosition + DRAW_DISTANCE/2;
    rainShaderProgram->setVec3("offsets", glm::mod(offsets,DRAW_DISTANCE));
...
void setup(){
    ...
    rainShaderProgram = new Shader("shaders/pointShader.vert", "shaders/shader.frag");
    //std::cout << "hello \n";
    ...
    //raindrops
    raindrops.VAO = createRainDrops(raindropCount);
    raindrops.vertexCount = raindropCount;
}
...
unsigned int createRainDrops(unsigned int count){
    ...
    for (int i = 0; i < count; i += 2) {
        ...
        float offsetX = ((float) (rand()) / max_rand - .5f) * 10.0f;
        float offsetY = ((float) (rand()) / max_rand - .5f) * 10.0f;
        float offsetZ = ((float) (rand()) / max_rand - .5f) * 10.0f;
    ...
    unsigned int VAO = createVertexArray(vertices, colors, indices, rainShaderProgram);
