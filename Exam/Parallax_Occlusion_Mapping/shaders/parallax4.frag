...
void main()
{
    ...
    float ambientOcclusion = texture(texture_ambient1, texCoords).r;
    ...
    float dc = max(0.0, dot(-fs_in.LightDir_tangent, N));

    float shadow = 0;
    if (dc > 0)
    {
        shadow = ShadowCalc(texCoords, fs_in.LightDir_tangent) * dc;
    }

    // TODO exercise 12.1 - use the shadow value to modulate the diffuse and specular colors of the fragment
    FragColor = vec4(ambient * ambientOcclusion + (diffuse + specular) * ambientOcclusion * shadow, albedo.a);
}